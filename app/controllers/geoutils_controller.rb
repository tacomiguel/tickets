  ...
  def sync
    ...
    if klass == User
      ...
      begin
        ...
        Rails.logger.tagged('GEOUTILS:SYNC_USER', 'FAILED_SYNC') do 
          Rails.logger.error data
        end
        render json: { message: "request faied", errors: instance.errors, record: instance, record_type: instance.class.name }, status: :baq_request
      end
